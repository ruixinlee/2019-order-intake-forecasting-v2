...
def Create_Historical_Glide_Paths(df_VISTA_master, df_SPRINT_master, country, CHD_reporting_date_cutoff = '2018-02-28'):
        df_SPRINT = df_SPRINT_master
        df_SPRINT = df_SPRINT[df_SPRINT['LEVEL_3_REPORTING'] == country]
        for col in ['ACTUAL_DATE', 'LEVEL_5_REPORTING', 'LEVEL_3_REPORTING', 'DAILYPREDICTEDRETAILS']:
            if df_SPRINT[col]:
                df_SPRINT = df_SPRINT.drop(col, axis=1)
        df_SPRINT = df_SPRINT.rename(columns = {'reporting_date': 'CHD_reporting_date'})

        df_VISTA = df_VISTA_master
        df_VISTA = df_VISTA[df_VISTA['LEVEL_3_REPORTING'] == country]
        Level_5_Reporting = df_VISTA['LEVEL_5_REPORTING'].unique()[0]

        #Get the sales EOM date. Use all data, regardless of model, to spot spikes/troughs.
        df_VISTA = df_VISTA.drop_duplicates('ORDER_NO')
        vista_eom = aggregte_orders(df_VISTA, ['LEVEL_5_REPORTING', 'LEVEL_3_REPORTING', 'CUSTOMER_HANDOVER_DATE'])
        vista_final_eom = find_eom(vista_eom, country, 'CUSTOMER_HANDOVER_DATE')
        #vista_final_eom.to_csv('vista_final_eom.csv')

        #Create Glide Path
        vista_curves = aggregte_orders(df_VISTA,
        ...
        # write_cut_off_point_comparision(vista_curves, df_VISTA, vista_final_eom)
        vista_curves = pd.merge(vista_curves, df_SPRINT, on=['BRAND', 'MODEL_FAMILY', 'CHD_reporting_date'], how='left')
    ...
    return(vista_curves, Level_5_Reporting)